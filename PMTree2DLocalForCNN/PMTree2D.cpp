...
namespace pmtree {
	...
	*/
	...
	bool PMTree2D::generateGeometry(RenderManager* renderManager) {
		...
		if (generateGeometry(renderManager, modelMat, length, width, root, glm::vec3(), glm::vec3(), vertices)) underground = true;
		renderManager->addObject("tree", "", vertices, true);

		return underground;
	}

	bool PMTree2D::generateGeometry(RenderManager* renderManager, const glm::mat4& modelMat, float segment_length, float segment_width, boost::shared_ptr<TreeNode>& node, glm::vec3& p0, glm::vec3& p1, std::vector<Vertex>& vertices) {
		bool underground = false;
		...
		float w1 = segment_width * (NUM_SEGMENTS - node->index) / NUM_SEGMENTS;
		if (node->index == 0) {
			p0 = glm::vec3(mat * glm::vec4(-w1 * 0.5f, 0, 0, 1));
			p1 = glm::vec3(mat * glm::vec4(w1 * 0.5, 0, 0, 1));
		}

		float w2 = segment_width * (NUM_SEGMENTS - node->index - 1) / NUM_SEGMENTS;
		...
		if (node->children.size() >= 1) {
			// extend the segment
			generateGeometry(renderManager, mat, segment_length, segment_width, node->children[0], pts[3], pts[2], vertices);
		}
		
		if (node->children.size() >= 2) {
			// branching
			generateGeometry(renderManager, mat, segment_length * node->children[1]->attenuationFactor, w1 * node->children[1]->attenuationFactor, node->children[1], glm::vec3(), glm::vec3(), vertices);
		}
	...
	void PMTree2D::generateTrainingData(const glm::mat4& modelMat, float segment_length, boost::shared_ptr<TreeNode>& node, const cv::Mat& imagePadded, int padding, Camera* camera, int screenWidth, int screenHeight, std::vector<cv::Mat>& localImages, std::vector<std::vector<float> >& parameters) {
		...
		// cropping sizeを計算
		float crop_size = glm::length(pp2 - pp);
